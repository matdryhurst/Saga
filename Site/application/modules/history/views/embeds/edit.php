	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<div class="control-group<?php echo form_error('count') ? ' error' : ''; ?>">
				...
				<div class='controls'>
					<input id='count' type='text' name='count' maxlength='11' value="<?php echo set_value('count', isset($history->count) ? $history->count : ''); ?>" disabled/>
			...
			<div class="control-group<?php echo form_error('embedid') ? ' error' : ''; ?>">
				<?php echo form_label('EmbedName', 'embedid', array('class' => 'control-label')); ?>
				<div class='controls' id='embedid'>
					<!-- <input id='embedid' type='text' name='embedid' maxlength='11' value="<?php echo set_value('embedid', isset($history->embedid) ? $history->embedid : ''); ?>" disabled/> -->
				 	<input id='embedName' type='text' value="<?php echo $embedName ?>" disabled/>
			...
			<div class="control-group<?php echo form_error('isedited') ? ' error' : ''; ?>">
				<div class='controls'>
					<label class='checkbox' for='isedited'>
						<input type='checkbox' id='isedited' name='isedited'  value='1' <?php echo set_checkbox('isedited', 1, isset($history->isedited) && $history->isedited == 1); ?> disabled/>
                        'Isedited'
					</label>
		...
		<fieldset class='form-actions'>
			<input type='submit' name='save' class='btn btn-primary' value="<?php echo lang('history_action_edit'); ?>" />
			<?php echo lang('bf_or'); ?>
			<?php echo anchor(SITE_AREA . '/embeds/history', lang('history_cancel'), 'class="btn btn-warning"'); ?>
			
			<?php if ($this->auth->has_permission('History.Embeds.Delete')) : ?>
				<?php echo lang('bf_or'); ?>
				<button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('history_delete_confirm'))); ?>');">
					<span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('history_delete_record'); ?>
				</button>
			<?php endif; ?>
		</fieldset>
    <?php echo form_close(); ?>
