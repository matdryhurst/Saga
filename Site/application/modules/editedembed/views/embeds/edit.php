	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<div class="control-group<?php echo form_error('sourceurl') ? ' error' : ''; ?>">
				...
				<div class='controls'>
					<input id='sourceurl' type='text' name='sourceurl' maxlength='100' value="<?php echo set_value('sourceurl', isset($editedembed->sourceurl) ? $editedembed->sourceurl : ''); ?>" />
			...
			<div class="control-group<?php echo form_error('embedid') ? ' error' : ''; ?>">
				...
				<div class='controls'>
					<input id='embedid' type='text' name='embedid' maxlength='11' value="<?php echo set_value('embedid', isset($editedembed->embedid) ? $editedembed->embedid : ''); ?>" />
		...
		<fieldset class='form-actions'>
			<input type='submit' name='save' class='btn btn-primary' value="<?php echo lang('editedembed_action_edit'); ?>" />
			<?php echo lang('bf_or'); ?>
			<?php echo anchor(SITE_AREA . '/embeds/editedembed', lang('editedembed_cancel'), 'class="btn btn-warning"'); ?>
			
			<?php if ($this->auth->has_permission('EditedEmbed.Embeds.Delete')) : ?>
				<?php echo lang('bf_or'); ?>
				<button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('editedembed_delete_confirm'))); ?>');">
					<span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('editedembed_delete_record'); ?>
				</button>
			<?php endif; ?>
		</fieldset>
    <?php echo form_close(); ?>
